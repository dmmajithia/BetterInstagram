...
def follow():
	...
	#db = pymysql.connect(host = "localhost", user = "root", password = "123456", db = "dbbig")
	cursor = db.cursor()
	sql1 = "INSERT INTO relationship (user_appid, follower_appid) VALUES( %s, %s)"
	sql2 = "UPDATE user SET num_of_following =  num_of_following + 1 WHERE appID = %s"
	sql3 = "UPDATE user SET num_of_follower =  num_of_follower + 1 WHERE appID = %s" 
	try:
		cursor.execute(sql1, (appID, appID2))
		cursor.execute(sql2, appID)
		cursor.execute(sql3, appID2)
		db.commit()
		success = True
	except:
		db.rollback()
...
def unfollow():
	...
	#db = pymysql.connect(host = "localhost", user = "root", password = "123456", db = "dbbig")
	cursor = db.cursor()
	sql1 = "DELETE FROM relationship WHERE (user_appid= %s AND follower_appid= %s)"
	sql2 = "UPDATE user SET num_of_following =  num_of_following - 1 WHERE appID = %s"
	sql3 = "UPDATE user SET num_of_follower =  num_of_follower - 1 WHERE appID = %s"
	try:
		cursor.execute(sql1, (appID, appID2))
